...
public abstract class DoubleAnimationBase : AnimationTimeline {
  internal DoubleAnimationBase(IntPtr cPtr, bool cMemoryOwn) : base(cPtr, cMemoryOwn) {
  }

  internal static HandleRef getCPtr(DoubleAnimationBase obj) {
  ...
  protected internal abstract float GetCurrentValueCore(float defaultOriginValue, float defaultDestinationValue, AnimationClock animationClock);

  internal new static IntPtr Extend(string typeName) {
    return NoesisGUI_PINVOKE.Extend_DoubleAnimationBase(Marshal.StringToHGlobalAnsi(typeName));
  }
}
